#Blue Charm Beacons IOS SDK Instruction DOC（English）

----
## 1. Introduction
With this SDK, you can scan and configure Blue Charm Beacon devices. The SDK include follow main class:
* KBeaconsMgr: Global definition, responsible for scanning Blue Charm Beacon device advertisement packets, and monitoring the Bluetooth status of the system;

* KBeacon: An instance of a Blue Charm Beacon device, KBeaconsMgr creates an instance of KBeacon when it finds a physical device. Each KBeacon instance has three properties: KBAdvPacketHandler, KBAuthHandler, KBCfgHandler.
...
in this stage, KBeaconsMgr will scan and parse the advertisement packet about Blue Charm Beacon devices, and it will create "KBeacon" instance for every founded devices, developers can get all advertisements data by its allAdvPackets or getAdvPacketByType function.
...
* KBeaconProDemo_Ios: The app can scan Blue Charm Beacon devices and configure iBeacon related parameters.
...
func printScanPacket(_ advBeacon: KBeacon)
{
    ...
    for advPacket in allAdvPackets
    {
        ...
        case KBAdvType.IBeacon:
            ...
            //notify: this is not standard iBeacon protocol, we get minor ID from private
...
After start scanning, The KBeaconMgr will buffer all found Blue Charm Beacon device. If the app want to remove all buffered Blue Charm Beacon device, the app can:  
...
//When the app is connected to the Blue Charm Beacon device, the app can specify which the configuration parameters to be read,
...
The push button trigger feature is used in some hospitals, nursing homes and other scenarios. When the user encounters some emergency event(SOS button), they can click the button and the Blue Charm Beacon device will start broadcast or the Blue Charm Beacon device send the click event to connected Android/IOS app.
...
In some scenarios, our app will always be connected to the Blue Charm Beacon device. We need the app can receive a press notification event when the button is pressed.  
...
##### 4.3.6.2 Read sensor events records
  ...
  * KBSensorReadOption.NewRecord:  read history records and move next. After app reading records, the Blue Charm Beacon device will move the pointer to the next unreaded record. If the app send read request again, the Blue Charm Beacon device sends next unread records and move the pointer to next.
...
#### 4.3.7.1 Ring device
 For some Blue Charm Beacon device that has buzzer function. The app can ring device. For ring command, it has 5 parameters:
 ...
 * KBFirmwareDownload: Responsible for download the JSON or firmware from Blue Charm cloud.
 * DFUService: This DFU service that implementation Nordic's DFU library.
 ![avatar](https://github.com/BlueCharmBeacons/BlueCharm_KBeaconProDemo_Android/blob/master/kbeacon_dfu__ios_arc.png)

 ### 5.1 Add DFU function to the application.
 Edit Podfile:
 The DFU Demo need download the firmware from Blue Charm cloud by AFNNetworking library.  Also the DFU demo using nordic DFU library for update.
  ```
  platform :ios, '11.0'
  use_frameworks!
